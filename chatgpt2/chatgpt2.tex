...
\begin{itemize}
	\item \textbf{Transformeur}.\index{transformeur} (\emph{Transformer.}) C'est la succession de deux types de couches qui regroupe le principal travail du réseau.
	...
	\item \textbf{Flux résiduel.}\index{flux@flux résiduel} (\emph{Residual stream.})
...
Le vocable \emph{retranscription}\index{retranscription} est la traduction officieuse du terme \emph{unembedding}.

Rappelons que le \defi{plongement}\index{plongement} (\emph{embedding}) est une application linéaire 
...
Le vecteur $x = (x_1, \ldots, x_N) \in \Rr^N$ s'appelle le vecteur \defi{logit}\index{logit}, ses coordonnées $x_i$ sont des nombres réels qu'il reste difficile à interpréter.
...
\index{produit tensoriel}\index{tenseur}
...
Si on trouve le mot \mot{appartement} (qui joue le rôle du mot1) auparavant dans le texte alors voici différentes propositions pour compléter la phrase :
...
\mybox{
	$(A \otimes I) x = \operatorname{softmax} \big( x^T W_Q^T W_K x \big)$
}
où 
\begin{itemize}
...
Rappelons que la \defi{loi de Gauss centrée réduite}\index{loi de Gauss} est définie par :
...
La fonction \defi{GELU}\index{GELU}\index{fonction d'activation} (pour \emph{Gauss Error Linear Unit}) est définie par 
$$\GELU(x) = x \Phi(x)$$
