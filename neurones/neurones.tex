...
On peut donc définir ce qu'est un perceptron. Un \defi{perceptron linéaire} à $n$ variables et de fonction d'activation la fonction marche de Heaviside est la donnée de $n$ coefficients réels $a_1,\ldots,a_n$ auxquels sont associés la fonction $F : \Rr \to \Rr$ définie par $F = H \circ f$, c'est-à-dire :
$$\begin{cases}
...
Trouvons un droite qui les sépare. Par exemple, la droite d'équation $4x-y=0$ sépare les ronds des carrés. On définit donc le neurone avec les poids $a=4$ et $b=-1$.
...
Par exemple, si $x=0$ et $y=1$ alors, on calcul $1\cdot0+1\cdot1-1 = 0 \ge0$. Composée avec la fonction marche de Heaviside, la fonction $F(x,y)$
...
On rappelle qu'un ensemble $A \subset \Rr^2$ découpe le plan en trois parties disjointes :
...
\index{theoreme@théorème d'approximation universel}

Nous pouvons maintenant énoncer le résultat théorique le plus important de ce chapitre.

\begin{theoreme}[Théorème d'approximation universel]
