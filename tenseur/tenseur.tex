...
\begin{itemize}
  ...
  \item \ci{T + TT} calcule chaque somme $T_{ijk}+T'_{ijk}$ (ici en dimensions 3).
\end{itemize}
...
L'instruction est \ci{T.flatten()}\index{numpy@\numpy!flatten@\ci{flatten()}} qui pour notre exemple renvoie le tenseur de taille $(18,)$ :
...
Ce n'est qu'à la fin, lorsque l'on calcule $f(7)$ que l'emplacement mémoire de \ci{x} est affectée à  la valeur \ci{[7]} et que le calcul $y=x+2$ est vraiment effectué. 
